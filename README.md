...
- Responder publicaciones externas (modo comentario):
  - En el bot de Telegram envía `/comentar <texto del post>`. Primero evalúa si vale la pena participar (solo responde si aporta valor real para el ICP); luego pasa por el contrato/ICP, el validador externo y devuelve un único comentario en bloque `<code>`, cerrado con una pregunta para seguir la conversación.
